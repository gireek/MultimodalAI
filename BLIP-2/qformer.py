...
class QFormerBlock(nn.Module):
    ...
    """
    def __init__(self, d_model=384, nhead=4, dim_feedforward=512):
        super().__init__()
...
class QFormer(nn.Module):
    ...
    """
    def __init__(
        self,vocab_size,num_queries=32,d_model=384,nhead=4,num_layers=3,dim_feedforward=1024):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            QFormerBlock(d_model, nhead, dim_feedforward) for _ in range(num_layers)
        ])
    ...
    def forward(self, question_ids, image_embeds):
        ...
        query_embs = self.query_embeddings.unsqueeze(0).expand(B, -1, -1)
...
class VisualQueryBridge(nn.Module):
    """
    We want to feed Q-Former outputs into GPT-2 as 'prefix embeddings'.
    That means we map [B, N, d_model_qformer] into [B, N, d_model_gpt2].
    Then at inference/training time, we can prepend these embeddings to
    the question tokens in GPT-2's embedding space.
    """
    def __init__(self, d_qformer=384, d_gpt2=768, num_queries=32):
        super().__init__()
        self.num_queries = num_queries
        self.proj = nn.Linear(d_qformer, d_gpt2)

    def forward(self, qformer_outputs):
        out = self.proj(qformer_outputs)  # [B, N, d_gpt2]
        return out